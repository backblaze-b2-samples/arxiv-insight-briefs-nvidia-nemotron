...
const OPTIONAL_VARS = ["NVIDIA_API_KEY"];
...
function checkEnv() {
  ...
  // will gracefully degrade rather than fail outright.
  for (const k of OPTIONAL_VARS) {
    if (!env[k]) {
      warn(
        `${k} is not set`,
        "ok — the brief pipeline will skip LLM stages and produce a 'no analysis' brief. Get a free key at https://build.nvidia.com to enable synthesis.",
      );
    }
  }
}
