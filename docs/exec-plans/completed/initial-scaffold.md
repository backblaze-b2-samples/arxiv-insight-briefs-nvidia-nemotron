...
| `app/repo/b2_client.py` (boto3 + `@functools.lru_cache` client) — pattern is great, contents need surgery | `b2_client.py` uses `user_agent_extra="b2ai-oss-start"`, reads `settings.b2_s3_endpoint` / `b2_application_key_id`, and **omits `region_name`** — all three diverge from parent CLAUDE.md | Rewritten `app/repo/b2_client.py` with `user_agent_extra="arxiv-insight-briefs/0.1.0 (backblaze-b2-samples)"`, `region_name=settings.b2_region`, `endpoint_url=settings.b2_endpoint`, `aws_access_key_id=settings.b2_key_id`. New helpers: `head_object_or_none`, `put_bytes`, `get_text`, `list_prefix` |
...
| B2 env var: endpoint | `B2_S3_ENDPOINT` | `B2_ENDPOINT` *(also rename `settings.b2_s3_endpoint` → `settings.b2_endpoint` in `app/config/settings.py` + every reader)* |
| B2 env var: key id | `B2_APPLICATION_KEY_ID` | `B2_KEY_ID` *(also rename `settings.b2_application_key_id` → `settings.b2_key_id`)* |
