...
| `useHealth` query has no per-request `AbortSignal.timeout(5000)` | A hung `/health` request blocks until TanStack's refetch interval retriggers; the next tick fires a fresh request but the prior one isn't actively cancelled | Re-add `signal: AbortSignal.timeout(5000)` to the `fetch` call inside `useHealth` (queries.ts:54) so each poll has a hard cap independent of refetch cadence | Low | Open |
