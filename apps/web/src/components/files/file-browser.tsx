...
// Module-scope so React Compiler treats it as a stable element.
function FileTypeIcon({
  ...
  if (contentType.startsWith("text/")) return <FileTextIcon className={className} />;
  if (contentType === "application/json") return <FileTextIcon className={className} />;
...
interface TreeRowProps {
  ...
  onInspect: (file: FileMetadata) => void;
...
export function FileBrowser() {
  ...
  const [inspectFile, setInspectFile] = useState<FileMetadata | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<FileMetadata | null>(null);

  const tree = useMemo(() => buildFileTree(files), [files]);

  // Auto-expand top-level folders once on first data arrival; preserve
  // the user's choices on subsequent refreshes.
  useEffect(() => {
  ...
  return (
    <>
      <Card>
        ...
        <CardContent className="p-3">
          {isLoading ? (
            ...
            <EmptyState
              icon={FolderOpen}
              title="This bucket prefix is empty"
              description="Submit a brief to populate papers/ and briefs/."
            />
          ) : (
      ...
      <FileMetadataPanel
        file={inspectFile}
        open={!!inspectFile}
        onOpenChange={(open) => !open && setInspectFile(null)}
      />
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete <strong>{deleteTarget?.filename}</strong> from B2.
              No guard is applied — cached PDFs under <code>papers/</code> can be removed too.
